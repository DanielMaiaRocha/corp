...
const setCookies = (res, acessToken, refreshToken) => {
  res.cookie("acessToken", acessToken, {
    httpOnly: true, // prevenção contra XSS
  ...
  try {
    ...
    if (refreshToken) {
      try {
        ...
        console.log(
          " Refresh token removed from Redis for user:",
          decoded.userId
        );
      } catch (error) {
        console.log(
          " Error verifying refresh token on logout:",
          error.message
        );
      }
    }
...
export const refreshToken = async (req, res) => {
  try {
    ...
    if (storedToken !== refreshToken) {
      console.log(" Token mismatch: Received does not match stored in Redis");
      return res.status(403).json({ message: "Invalid refresh Token" });
    }

    // Se passou por todas as verificações, gera um novo accessToken
